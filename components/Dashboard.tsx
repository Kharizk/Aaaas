...
export const Dashboard: React.FC<DashboardProps> = ({ products, units, switchToTab, onNavigateToList, currentUser }) => {
  ...
  const SAPTile = ({ title, value, subValue, icon: Icon, onClick, colorClass }: any) => (
    <div onClick={onClick} className="relative overflow-hidden bg-white/80 backdrop-blur-xl border border-white/80 rounded-[2rem] shadow-sm hover:shadow-[0_20px_40px_-12px_rgba(0,0,0,0.15)] hover:border-sap-primary/20 hover:-translate-y-1.5 transition-all duration-500 cursor-pointer group h-40">
      <div className={`absolute inset-0 bg-gradient-to-br ${colorClass} opacity-[0.03] group-hover:opacity-[0.06] transition-opacity duration-500`}></div>
      <div className={`absolute -right-8 -bottom-8 w-40 h-40 rounded-full bg-gradient-to-br ${colorClass} opacity-10 group-hover:opacity-20 blur-2xl transition-all duration-700`}></div>
      <div className={`absolute -left-6 -bottom-6 opacity-[0.04] group-hover:opacity-[0.1] transition-all duration-500 text-black group-hover:-rotate-6 group-hover:scale-110 drop-shadow-sm`}><Icon size={140} /></div>
      <div className="p-7 flex flex-col h-full justify-between relative z-10">
          <div className="flex justify-between items-start">
            <div className={`p-3 rounded-2xl bg-white shadow-md shadow-gray-200/50 text-gray-700 group-hover:text-white group-hover:bg-gradient-to-br ${colorClass} transition-all duration-500 transform group-hover:scale-110`}><Icon size={24} strokeWidth={2.5} /></div>
            {subValue && <span className="text-[11px] font-black bg-white border border-gray-100 text-gray-500 px-3 py-1 rounded-full shadow-sm">{subValue}</span>}
          </div>
          <div className="mt-2">
            <div className="text-3xl font-black text-gray-900 tracking-tight font-mono group-hover:bg-clip-text group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-gray-900 group-hover:to-gray-600 transition-all">{value}</div>
            <div className="text-xs font-bold text-gray-500 mt-1">{title}</div>
          </div>
      </div>
    </div>
  );
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {/* Low Stock Widget */}
            <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 border border-amber-100 rounded-[2rem] shadow-sm p-6">
                <h3 className="font-black text-amber-900 mb-5 flex items-center gap-2 text-sm"><AlertTriangle size={18} strokeWidth={2.5} className="text-amber-600"/> تنبيهات المخزون المنخفض</h3>
                <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                    {products.filter(p => (p.stock || 0) <= (p.lowStockThreshold || 0) && (p.lowStockThreshold || 0) > 0).map(p => (
                        <div key={p.id} className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-amber-50 hover:bg-white hover:shadow-md hover:-translate-y-0.5 hover:border-amber-200 transition-all duration-300">
                            <div>
                                <div className="font-bold text-gray-800 text-xs mb-0.5">{p.name}</div>
                                <div className="text-[10px] text-gray-500 font-mono font-medium">المتوفر: <span className="text-red-500 font-black">{p.stock || 0}</span> / الحد: {p.lowStockThreshold}</div>
                            </div>
                            <span className="bg-gradient-to-br from-amber-100 to-amber-200 text-amber-800 px-3 py-1 rounded-xl text-[10px] font-black shadow-sm ring-1 ring-amber-300/30">منخفض</span>
                        </div>
                    ))}
                    {products.filter(p => (p.stock || 0) <= (p.lowStockThreshold || 0) && (p.lowStockThreshold || 0) > 0).length === 0 && (
                        <div className="text-center text-amber-400 text-xs py-10 flex flex-col items-center justify-center gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-amber-100/50 flex items-center justify-center border border-amber-200/50">
                                <CheckCircle size={24} strokeWidth={2} className="text-amber-500" />
                            </div>
            ...
            {currentShift && (
                <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 border border-blue-100 rounded-[2rem] shadow-sm p-6 mt-6">
                    <h3 className="font-black text-blue-900 mb-5 flex items-center gap-2 text-sm"><Clock size={18} strokeWidth={2.5} className="text-blue-600"/> ملخص الوردية الحالية</h3>
                    <div className="space-y-3">
                        <div className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-blue-50">
                            <span className="text-xs text-gray-500 font-bold">وقت البدء</span>
                            <span className="text-xs font-mono font-black text-blue-700 bg-blue-100 px-2 py-1 rounded-lg">{new Date(currentShift.startTime).toLocaleTimeString('ar-SA')}</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-blue-50">
                            <span className="text-xs text-gray-500 font-bold">الكاشير</span>
                            <span className="text-xs font-bold text-gray-800 flex items-center gap-2"><UserIcon size={12} className="text-gray-400"/> {currentShift.userName}</span>
                        </div>
                        <div className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-blue-50">
                            <span className="text-xs text-gray-500 font-bold">المبيعات (تقريبي)</span>
                            <span className="text-sm font-mono font-black text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg ring-1 ring-emerald-200">
                                {salesData
            ...
            <div className="bg-gradient-to-br from-purple-50 to-purple-100/50 border border-purple-100 rounded-[2rem] shadow-sm p-6 mt-6">
                <h3 className="font-black text-purple-900 mb-5 flex items-center gap-2 text-sm"><Crown size={18} strokeWidth={2.5} className="text-purple-600"/> كبار العملاء</h3>
                <div className="space-y-3">
                    {Object.values(salesData.reduce((acc: any, sale) => {
                        ...
                        <div key={c.id} className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-purple-50 hover:bg-white hover:shadow-md hover:-translate-y-0.5 hover:border-purple-200 transition-all duration-300">
                            <div className="flex items-center gap-4">
                                <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black shadow-sm ${idx === 0 ? 'bg-gradient-to-br from-yellow-300 to-yellow-500 text-yellow-900 ring-2 ring-yellow-200' : 'bg-purple-100 text-purple-700'}`}>
                                    {idx + 1}
                                </div>
                                <div>
                                    <div className="font-bold text-gray-800 text-xs mb-0.5">{c.name}</div>
                                    <div className="text-[10px] text-gray-500 font-mono font-medium">{c.count} عملية شراء</div>
                                </div>
                            </div>
                            <span className="font-black text-purple-700 text-sm font-mono">{c.total.toLocaleString()}</span>
                        </div>
                    ))}
                    {salesData.length === 0 && (
                        <div className="text-center text-purple-400 text-xs py-10 flex flex-col items-center justify-center gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-purple-100/50 flex items-center justify-center border border-purple-200/50">
                                <Crown size={24} strokeWidth={2} className="text-purple-400" />
                            </div>
            ...
            {/* Top Products Widget */}
            <div className="bg-gradient-to-br from-indigo-50 to-indigo-100/50 border border-indigo-100 rounded-[2rem] shadow-sm p-6 mt-6">
                <h3 className="font-black text-indigo-900 mb-5 flex items-center gap-2 text-sm"><ShoppingBag size={18} strokeWidth={2.5} className="text-indigo-600"/> المنتجات الأكثر مبيعاً</h3>
                <div className="space-y-3">
                    {Object.values(salesData.reduce((acc: any, sale) => {
                        ...
                        <div key={p.id} className="flex justify-between items-center p-3 bg-white/80 rounded-2xl border border-indigo-50 hover:bg-white hover:shadow-md hover:-translate-y-0.5 hover:border-indigo-200 transition-all duration-300">
                            <div className="flex items-center gap-4">
                                <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black shadow-sm ${idx === 0 ? 'bg-gradient-to-br from-yellow-300 to-yellow-500 text-yellow-900 ring-2 ring-yellow-200' : 'bg-indigo-100 text-indigo-700'}`}>
                                    {idx + 1}
                                </div>
                                <div>
                                    <div className="font-bold text-gray-800 text-xs mb-0.5">{p.name}</div>
                                    <div className="text-[10px] text-gray-500 font-mono font-medium">{p.count} قطعة مباعة</div>
                                </div>
                            </div>
                            <span className="font-black text-indigo-700 text-sm font-mono">{p.total.toLocaleString()}</span>
                        </div>
                    ))}
                    {salesData.length === 0 && (
                        <div className="text-center text-indigo-400 text-xs py-10 flex flex-col items-center justify-center gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-indigo-100/50 flex items-center justify-center border border-indigo-200/50">
                                <ShoppingBag size={24} strokeWidth={2} className="text-indigo-400" />
                            </div>
        ...
        <div className="flex flex-col gap-6">
            <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[300px] group border border-slate-700">
                <div className="absolute top-0 right-0 w-80 h-80 bg-sap-primary/30 rounded-full -mr-20 -mt-20 blur-[80px] group-hover:bg-sap-primary/40 group-hover:scale-110 transition-all duration-1000"></div>
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-sap-secondary/20 rounded-full blur-[60px] group-hover:bg-sap-secondary/30 transition-all duration-1000"></div>
                
                <div className="relative z-10">
                    <div className="w-14 h-14 bg-white/10 rounded-[1.2rem] flex items-center justify-center backdrop-blur-md mb-6 border border-white/20 shadow-[0_8px_16px_rgba(0,0,0,0.2)] group-hover:scale-110 transition-transform duration-500">
                        <Zap size={28} className="text-sap-secondary drop-shadow-[0_0_10px_rgba(197,160,89,0.5)] fill-current" />
                    </div>
                    <h3 className="text-4xl font-black mb-3 leading-tight tracking-tight">جاهز للبدء؟</h3>
                    <p className="text-slate-400 text-base leading-relaxed font-bold">افتح الكاشير وابدأ ببيع منتجاتك الآن بواجهة سريعة وعصرية.</p>
                </div>
                <div className="relative z-10 space-y-3 mt-8">
                    <button onClick={() => switchToTab('pos')} className="w-full py-4 bg-white text-slate-900 rounded-2xl font-black text-sm hover:bg-slate-50 shadow-[0_8px_30px_rgba(255,255,255,0.1)] hover:shadow-[0_12px_40px_rgba(255,255,255,0.2)] transition-all flex items-center justify-center gap-3 transform hover:-translate-y-1">
                        <ShoppingBag size={18} strokeWidth={2.5}/> فتح الكاشير
                    </button>
                </div>
            </div>
